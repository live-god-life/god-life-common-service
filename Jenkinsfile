pipeline {
    agent any
    environment {
        imagename = "onezo/god-life-common-service"
    ...
    stages {
        stage('git clone') {
            steps {
                ...
                git url: 'git@github.com:live-god-life/god-life-common-service.git',
                branch: 'release',
        ...
        stage('Run Docker Deploy Script') {
            steps {
                ...
                sh "ssh -o StrictHostKeyChecking=no root@106.10.40.232 -p 2222 '/root/DeployScript/common-service-deploy.sh'"
            }
            post {
                success {
